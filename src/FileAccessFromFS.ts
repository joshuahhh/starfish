...
import { FileAccess, FileMetadata } from "./FileAccess.js";

export class FileAccessFromFS implements FileAccess {
  constructor(
...
export async function selectLocalDirectory(): Promise<FileAccessFromFS | null> {
  try {
    const handle = await window.showDirectoryPicker({ mode: "readwrite" });
    await saveDirectoryHandle(handle);
    return new FileAccessFromFS(handle, "realfs");
  } catch (err) {
...
export async function opfsFileAccess(): Promise<FileAccessFromFS> {
  const handle = await navigator.storage.getDirectory();
  return new FileAccessFromFS(handle, "opfs");
}
...
export async function restoreDirectoryHandle(): Promise<FileAccessFromFS | null> {
  try {
    const db = await openDB("FileAccessDB", 1, {
      upgrade(db) {
        if (!db.objectStoreNames.contains("handles")) {
